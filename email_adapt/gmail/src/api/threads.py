...
class GmailThreadExtractor:
    ...
    def __init__(self, email_address: str) -> None:
        """Initialize the extractor with email address."""
    ...
    def _setup_paths(self) -> None:
        ...
        # Define all paths
        self.credentials_path = self.root_dir / "credentials/gmail_credentials.json"
        self.credentials_user_dir = self.root_dir / "credentials" / safe_email
        self.data_user_dir = self.root_dir / "data" / safe_email
        ...
        for directory in (self.credentials_user_dir, self.data_user_dir):
            try:
                ...
                logger.info(f"[{self.__class__.__name__}] Created/verified directory at: {directory}")
            except Exception as e:
                logger.error(f"[{self.__class__.__name__}] Failed to create directory {directory}: {e}")
                raise
        ...
        for attempt in range(self.MAX_RETRIES):
            try:
                ...
                if attempt == self.MAX_RETRIES - 1:
                    logger.error(
                        f"[{self.__class__.__name__}] Failed to initialize Gmail service after {self.MAX_RETRIES} attempts"
                    )
                    raise
                logger.warning(f"[{self.__class__.__name__}] Attempt {attempt + 1} failed: {e}")
                continue
            except Exception as e:
                logger.error(f"[{self.__class__.__name__}] Unexpected error initializing Gmail service: {e}")
                raise
    ...
    def _get_credentials(self) -> Optional[Credentials]:
        ...
        try:
            ...
            logger.warning(f"[{self.__class__.__name__}] Failed to load existing token")
            pass
        ...
        if not creds or not creds.valid:
            try:
                ...
                logger.warning(f"[{self.__class__.__name__}] Failed to refresh token: {e}")
                # Force new token creation
                creds = None

            # If refresh failed or no existing credentials, create new ones
            if not creds:
            ...
            if creds:
                ...
                try:
                    ...
                    logger.error(f"[{self.__class__.__name__}] Failed to save token: {e}")
            else:
                logger.error(f"[{self.__class__.__name__}] Failed to retrieve and update user's token")
                return None
    ...
    def _extract_text_from_html(self, html_content: str) -> str:
        ...
        try:
            ...
            logger.warning(f"[{self.__class__.__name__}] Failed to parse HTML content: {e}")
            return html_content
    ...
    def get_threads(self, max_results: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"[{self.__class__.__name__}] Saved {len(detailed_threads)} threads to {self.data_path}")

            return detailed_threads

        except Exception as e:
            logger.error(f"[{self.__class__.__name__}] Error fetching threads: {e}")
            raise
    ...
    def _parse_message(self, message: Dict[str, Any]) -> Dict[str, Union[str, bool]]:
        ...
        raw_body = self._get_message_body(message["payload"])

        # Clean the body by removing quoted content
        cleaned_body = self._clean_message_body(raw_body)
    ...
    def _clean_message_body(self, body: str) -> str:
        """Clean the message body by removing quoted content.

        :param body: Raw message body text
        :return: Cleaned message body with only the latest content
        """
        # Common patterns that indicate quoted content
        patterns = [
            r"On .+?, at .+?, .+ wrote:",  # Matches "On [date], at [time], [name] wrote:"
            r"On .+ wrote:",  # Standard email quote
        ...
        # Split at the first occurrence of any quote pattern
        parts = re.split(combined_pattern, body, maxsplit=1)

        # Return the first part (latest message)
